...
test('Contact kinetic signal tide stays below content, responds to pointer and preserves accessible interaction', async ({ page }) => {
  ...
  const coverage = await grid.evaluate((element) => {
    ...
    return {
      ...
      lowerBand: fieldBounds.top > gridBounds.top + gridBounds.height * .24,
  ...
  expect(coverage.height).toBeGreaterThanOrEqual(350)
  expect(coverage.height).toBeLessThanOrEqual(470)
...
test('canonical design route isolates the exact approved hero', async ({ page }) => {
  ...
  expect(geometry.brand).toMatchObject({ right: 90, bottom: 78, width: 633.59375, height: 190.265625 })
  expect(geometry.mobile).toMatchObject({ right: -22, width: 297 })
