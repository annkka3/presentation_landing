...
export const cryptoReality = {
  ...
  sections: {
    ...
    archetypes: {
      ...
      items: [
        { name: 'Risk Manager', color: '#3d91ff', traits: [l('дисциплина', 'discipline'), l('банкролл', 'bankroll'), l('репутация', 'reputation')], text: l('Контроль риска, низкий FOMO и стабильный набор score через дисциплину.', 'Risk control, low FOMO, and steady score growth through discipline.') },
        { name: 'Meme-coin Degen', color: '#a653ff', traits: ['FOMO', l('импульс', 'impulse'), l('социальный шум', 'social noise')], text: l('Живёт на скорости, ловит тренды и платит стрессом за потенциальную альфу.', 'Lives on speed, chases trends, and pays in stress for potential alpha.') },
        { name: 'On-chain Detective', color: '#1fd6ff', traits: [l('данные', 'data'), l('проверка', 'verification'), l('сигналы', 'signals')], text: l('Сначала ищет подтверждение, затем принимает решение.', 'Looks for evidence before making the decision.') },
        { name: 'Leverage Cowboy', color: '#ffb936', traits: [l('риск', 'risk'), l('альфа', 'alpha'), l('волатильность', 'volatility')], text: l('Выбирает агрессивные сценарии и принимает сильную амплитуду результата.', 'Chooses aggressive scenarios and accepts a wider outcome range.') },
        { name: 'HODL Monk', color: '#64df62', traits: [l('терпение', 'patience'), l('низкий стресс', 'low stress'), l('устойчивость', 'resilience')], text: l('Сохраняет спокойствие и не реагирует на каждый рыночный шум.', 'Stays calm and does not react to every market noise.') },
        { name: 'Airdrop Farmer', color: '#f0d05b', traits: [l('рутина', 'routine'), l('система', 'system'), l('награды', 'rewards')], text: l('Собирает ценность через повторяемость, дисциплину и внимательность.', 'Extracts value through repeatability, discipline, and attention.') },
        { name: 'Moon Prophet', color: '#f96bdc', traits: [l('убеждение', 'conviction'), l('нарратив', 'narrative'), l('ожидание', 'expectation')], text: l('Сильно верит в сценарий и готов выдерживать неопределённость.', 'Strongly believes in the narrative and can withstand uncertainty.') },
        { name: 'Capitulation Doomer', color: '#ff5d5d', traits: [l('защита', 'defense'), l('сомнение', 'doubt'), l('снижение риска', 'risk-off')], text: l('Видит угрозы раньше других, но рискует пропустить восстановление.', 'Sees threats early, but risks missing the recovery.') },
      ],
    },
    finale: {
