    ...
    <Container>
      <div className={`hero-panels ${activePanel !== null ? 'has-active' : ''}`} role="group" aria-label={t.heroGroup} onMouseLeave={clearHover} onBlur={leaveHeroFocus}>
        {heroModes.map((mode, index) => {
          ...
          return <a id={mode.key} key={mode.key} className={`hero-panel ${isActive ? 'is-active' : ''} ${videoIsPlaying ? 'is-video-playing' : ''}`} data-state={isActive ? 'active' : 'idle'} href="#featured" onMouseEnter={() => activateHover(index)} onFocus={() => focusPanel(index)} onClick={(event) => mobileClick(event, index)} style={{ '--mode-accent': mode.accent } as React.CSSProperties}>
